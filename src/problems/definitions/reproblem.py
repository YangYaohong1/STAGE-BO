...
class Knee1():
    ...
    def evaluate(self, x):
        ...
        g = 1 + (9 / (n - 1)) * (x2 + x3)
        K = 1
        r = 5 + 10 * (x1 - 0.5) ** 2 + (1 / K) * np.cos(2 * K * np.pi * x1)
        f[0] = g * r * np.sin(np.pi * x1 / 2)
        f[1] = g * r * np.cos(np.pi * x1 / 2)

        return f

class Knee5():
    def __init__(self):
        self.problem_name = 'Knee5'
        self.n_objectives = 2
        self.n_variables = 3
        self.n_constraints = 0
        self.lbound = np.zeros(self.n_variables)
...
class CRE32():
    ...
    def evaluate(self, x):
        ...
        g[1] = -(x_L / x_D) + 15.0
        g[2] = -(x_L / x_T) + 19.0
        g[3] = 0.45 * np.power(DWT, 0.31) - x_T
        g[4] = 0.7 * x_D + 0.7 - x_T
        g[5] = 500000.0 - DWT
        g[6] = DWT - 3000.0
        g[7] = 0.32 - Fn
